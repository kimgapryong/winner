...
public class BossCreateSystem : MonoBehaviour
{
    ...
    {
        if(manager.score >= 180 && !isBossImageSpwanStarted && !GameManager.Instance.GameStage)
        {
            //나중에 다시 활성화 시켜야 할 것
        ...
        }else if(manager.score >= 450 && !isBossImageSpwanStarted && GameManager.Instance.GameStage)
        {
            enemycreate.SetActive(false);
